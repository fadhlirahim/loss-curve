...
/** Format for use inside a TeX formula — parenthesize negatives. */
const texNum = (n: number) => (n < 0 ? `(${fmt(n)})` : fmt(n))
const dL = (id: NodeId) => `\\frac{\\partial L}{\\partial ${id}}`

export type Step = {
  node: NodeId
  rule: string
  /** KaTeX source for the chain-rule arithmetic at this node, with live numbers. */
...
export const STEPS: Step[] = [
  ...
  },
  {
    ...
    formula: (e) => `${dL('t')} = f \\cdot ${dL('L')} = ${texNum(e.values.f)}`,
  ...
  },
  {
    node: 'f',
    rule: '× swaps the inputs',
    formula: (e) => `${dL('f')} = t \\cdot ${dL('L')} = ${texNum(e.values.t)}`,
  ...
  },
  {
    ...
    formula: (e) =>
      `${dL('d')} = (1 - t^2) \\cdot ${dL('t')} = ${texNum(1 - e.values.t ** 2)} \\times ${texNum(e.grads.t)} = ${texNum(e.grads.d)}`,
  ...
  },
  {
    ...
    formula: (e) => `${dL('u')} = 1 \\cdot ${dL('d')} = ${texNum(e.grads.u)}`,
  ...
  },
  {
    node: 'c',
    rule: '+ passes it through',
    formula: (e) => `${dL('c')} = 1 \\cdot ${dL('d')} = ${texNum(e.grads.c)}`,
  ...
  },
  {
    ...
    rule: '× swaps the inputs',
    formula: (e) =>
      `${dL('a')} = b \\cdot ${dL('u')} = ${texNum(e.values.b)} \\times ${texNum(e.grads.u)} = ${texNum(e.grads.a)}`,
  ...
  },
  {
    ...
    rule: '× swaps the inputs',
    formula: (e) =>
      `${dL('b')} = a \\cdot ${dL('u')} = ${texNum(e.values.a)} \\times ${texNum(e.grads.u)} = ${texNum(e.grads.b)}`,
